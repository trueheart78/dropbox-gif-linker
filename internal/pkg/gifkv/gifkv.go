...
// GifReader is the standard interface for talking to gifs
type GifReader interface {
	String() string
	URL() string
	Markdown() string
	Tags() string
...
func (e Entry) String() string {
	return fmt.Sprintf("[%v] [%v] %v (%v) [used: %d]", e.ID, e.Tags(), e.BaseName, humanize.Bytes(uint64(e.FileSize)), e.Count)
}

// Tags break down the directory
func (e Entry) Tags() string {
	tags := strings.Replace(e.Directory, string(os.PathSeparator), "", 1)
...
func (e Entry) URL() string {
	...
	u.Path = filepath.Join(e.RemotePath, url.QueryEscape(e.BaseName))
	return u.String()
}

// Markdown returns a publicly-accessible markdown-based url
func (e Entry) Markdown() string {
	return fmt.Sprintf("![%v](%v)", e.BaseName, e.URL())
}
