...
func TestSupportedCommands(t *testing.T) {
	...
	assert.Equal(t, [2]string{"repair", "r"}, repairCommands)
...
func TestMarkdownMode(t *testing.T) {
	...
	assert.False(MarkdownMode("repair"))
...
func TestRepair(t *testing.T) {
	...
	assert.True(Repair("repair"))
	assert.True(Repair("r"))
	assert.True(Repair(":repair"))
	assert.True(Repair(":r"))

	assert.False(Repair("url"))
	assert.False(Repair("md"))
	assert.False(Repair("help"))
	assert.False(Repair("exit"))
	assert.False(Repair("config"))
	assert.False(Repair("count"))
	assert.False(Repair("version"))
	assert.False(Repair("taylor"))
}
